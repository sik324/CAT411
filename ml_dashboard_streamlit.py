...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    colmap = {
        ...
        "pga": find_col(out, ["PGA_g", "pga_shakemap"], required=False),
...
with tab1:
    ...
    selected_points = plotly_events(map_fig, click_event=True, hover_event=False, select_event=False, key="bridge_map")
    st.info("Click a marker on the map to see bridge details below.")
    st.plotly_chart(map_fig, use_container_width=True)

    if selected_points:
        pt = selected_points[0]["pointIndex"]
        clicked_trace = selected_points[0]["curveNumber"]
        trace = map_fig.data[clicked_trace]
        customdata = trace.customdata[pt]
        clicked_id = str(customdata[0])

        match = filtered[filtered["Structure_ID"].astype(str) == clicked_id]
        if len(match) > 0:
            row = match.iloc[0]
            st.subheader(f"Clicked Bridge: {clicked_id}")
...
st.markdown("---")
st.caption("Observed damaged bridges are shown as solid markers. RF and GMM overpredictions are shown as open circles.")
